  ...
  <div class="classroomEditView">
    <div class="cac-bread-bar">
      <el-breadcrumb separator="/">
        ...
        <router-link v-else :to="{ path: source === 'platform' ? '/platformStudent' : '/studentClassroom' }">学生班级</router-link>
        <el-breadcrumb-item>{{status=="edit"?"修改班级":"创建班级"}}</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <!-- <div class="radiogroup" v-show="is_teach&&is_live">
    ...
    <LiveClass :params="params" :rules="rules" :optionSubject="optionSubject" :status="status" :optionCampus="optionCampus" :source="source" :isCharge="isCharge"></LiveClass>
  </div>
</template>
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        ...
        amount: [
          { required: this.isCharge >= 0 ? 'true' : 'false', message: '请填写收费标准', trigger: 'input' }
        ]
      },
  ...
  beforeRouteLeave (to, from, next) {
    if (to.path === '/platformStudent' || to.path === '/studentClassroom') {
    ...
    getClassDetail (id) {
      ...
      classroomApi.queryClassDetail(params).then(res => {
        if (res.ok) {
          ...
          this.params.class_status = res.data.class_status
          this.params.student_count = res.data.students.length
          ...
          if (this.isCharge >= 0 && res.data.course_type_id !== 5) {
            this.params.canSeeFee = true
            this.params.amount = res.data.charge_info.amount
            this.params.charge_method = res.data.charge_info.charge_method
            this.$set(this.params, 'withdraw_rulers_type', res.data.withdraw_rulers_type + '')
            this.$set(this.params, 'withdraw_rulers_amount', res.data.withdraw_rulers_amount + '')
            this.$forceUpdate()
            let arr = [{
              amount: '',
              refund_type: '0',
            ...
            if (res.data.option_charge.length > 0) {
              res.data.option_charge.map((v, i) => {
                if (v.charge_type * 1 === 1) {
                  arr[0] = {
                    amount: v.amount,
              ...
              this.params.option_charge = arr
            } else {
              this.params.option_charge = arr
            }
            if (this.params.withdraw_rulers_amount * 1 <= 0) {
              this.params.withdraw_rulers_amount = 1
            }
            if (this.params.withdraw_rulers_type * 1 <= 0) {
              this.params.withdraw_rulers_type = 1
            }
          }
        }
      })
