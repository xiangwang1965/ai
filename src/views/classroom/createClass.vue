<template>
  <div class="classWrape">
    ...
    <el-form ref="form" :rules="formRules" :model="form" size="mini">
      <el-form-item label-width="120px" prop="courseId" :label-position="formalign" label="选择课程">
        <el-select value-key="course" v-model="courseName" @change="changeCourse" placeholder="请选择">
          <el-option v-for="item in courseOptions" :key="item.id" :label="item.name" :value="item">
            <template>
              <span>{{item.name}}</span>
            </template>
          </el-option>
  ...
  data() {
    ...
    return {
      ...
      formRules: {
        ...
        name: [{ required: true, message: "请填写班级名字", trigger: "blur" }],
