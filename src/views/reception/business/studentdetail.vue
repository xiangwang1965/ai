<template>
  <div class="studentDetail" v-loading="loading">
    <div class="cac-bread-bar">
      <el-breadcrumb separator="/">
        <router-link v-if="source === 'student'" :to="{ path: '/student' }">学生管理</router-link>
        <router-link v-else :to="{ path: '/reception' }">业务办理</router-link>
    ...
    <div class="studentDetailContent" v-if="student.info">
      ...
      <div class="studentInfo" v-loading="loading1">
        ...
        <div>
          ...
          <span>{{(student.info.gender === 3 || student.info.gender === 0 )? '未知' : student.info.gender === 1 ? '男' : '女'}}</span>
        </div>
        <div>
        ...
        <div v-if="student.info.english_name">
      ...
      <div class="entryInfo" v-loading="loading2">
        ...
        <div v-for="(entry,index) in student.entryList" :key="index">
          <div style="height:30px;line-height:30px;margin-bottom:10px;">
            <p style="float:left;">
              <span v-if="entry.order_code" style="margin-right:20px;">
                <span class="infoLabel">订单编号：</span>
                <span>{{entry.order_code}}</span>
              </span>
              ...
              <span v-if="isCharge >= 0">
                <span class="infoLabel">{{(entry.all_remain_fee) < 0 ? '欠费':'余额'}}：</span>
                <span>{{(entry.all_remain_fee < 0 ? entry.all_remain_fee * -1 : entry.all_remain_fee) | twoFloatFormat}}元</span>
              </span>
            </p>
          ...
          <el-table :data="entry.data" max-width="1150">
            <el-table-column label="班级名称" min-width="100" prop="" show-overflow-tooltip>
            ...
            <el-table-column label="授课老师" prop="" show-overflow-tooltip>
              <template slot-scope="scope">
            ...
            <el-table-column label="开课时间" align="center" width="220" prop="" show-overflow-tooltip>
              <template slot-scope="scope">
                <span>{{scope.row.course_open_time ? scope.row.course_open_time : '无'}}</span>
              </template>
            </el-table-column>
            ...
            <el-table-column label="创建时间" prop="" width="100" show-overflow-tooltip>
              <template slot-scope="scope">
            ...
            <el-table-column align="center" fixed="right" v-if="isCharge >= 0" label="应收学费" prop="" width="100" show-overflow-tooltip>
              <template slot-scope="scope">
                <span>{{(scope.row.payable || 0) | twoFloatFormat}}元</span>
              </template>
            </el-table-column>
            <el-table-column align="center" fixed="right" v-if="isCharge >= 0" label="应收书本费" prop="" width="100" show-overflow-tooltip>
              <template slot-scope="scope">
                <span>{{(scope.row.payable_others || 0) | twoFloatFormat}}元</span>
              </template>
            </el-table-column>
            <el-table-column align="center" fixed="right" v-if="isCharge >= 0" label="实收" prop="" width="100" show-overflow-tooltip>
              <template slot-scope="scope">
                <span>{{(scope.row.pay + scope.row.pay_others || 0) | twoFloatFormat}}元</span>
              </template>
            </el-table-column>
        ...
        <pagination :config="detailPageConfig" @change="goDetailPage"></pagination>
      </div>
    </div>
    <el-dialog class="divideDialog" title="班级选择" :visible.sync="divideConfig.visible" @open="getClassList()" width="1150px">
      <div>
        <el-select v-model="searchConfig.teach_type" v-if="isWisroom >= 0" clearable placeholder="授课方式">
      ...
      <el-table
        ...
        <el-table-column prop="class_name" min-width="100" label="班级名称" show-overflow-tooltip>
        ...
        <el-table-column prop="course_type_name" label="课程类型" show-overflow-tooltip>
          <template slot-scope="scope">
            <span v-for="(courseType, i) in courseTypeList" :key="i" v-if="courseType.id * 1 === scope.row.course_type * 1">{{courseType.course_type_name}}</span>
            <span v-if="scope.row.course_type === 0">无</span>
          </template>
        </el-table-column>
        ...
        <el-table-column v-if="isWisroom !== 'wisroom'" prop="teach_type_name" label="授课方式" show-overflow-tooltip>
          <template slot-scope="scope">
            <span v-if="scope.row.teach_type == 2">面授</span>
            <span v-else>双师</span>
          </template>
        </el-table-column>
        <el-table-column v-else prop="teach_type_name" label="授课方式" show-overflow-tooltip>
          <template slot-scope="scope">
            <span>双师</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="收费标准" min-width="100" align="center" v-if="isCharge >= 0" show-overflow-tooltip>
          <template slot-scope="scope">
      ...
      <div slot="footer" class="footer">
        <el-button type="primary" size="small" class="cac-button-two cancel-button dialog_cancel_button" round @click="cancelSelect">取消</el-button>
        <el-button type="primary" size="small" class="cac-button-one dialog_confirm_button" round @click="doneSelectClass">确定</el-button>
      </div>
    </el-dialog>
  ...
  export default {
    ...
    methods: {
      ...
      getEntryInfo () {
        ...
        let params = {
          'student_id': this.student_id,
          page: this.detailPageConfig.page,
          per_page: this.detailPageConfig.per_page
        }
          ...
          if (res.status_code === 200) {
            ...
            res.data.list.map((v, i) => {
              this.$nextTick(() => {
                ...
                v.all_remain_fee = v.remain_fee + v.remain_fee_others
              })
            })
          }
          this.student.entryList = res.data.list
          this.detailPageConfig.total = res.data.total_count
        })
      },
      doneNext (type, data) {
        if (type === 'transfer') {
          this.$router.push({ name: this.source === 'student' ? 'studentChangeClass' : 'changeClass', query: {detail: true, class_id: data.class_id, student_id: this.student.info.id} })
        }
        if (type === 'quit') {
          this.$router.push({ name: this.source === 'student' ? 'studentQuitClass' : 'quitClass', query: {detail: true, class_id: data.class_id, enroll_id: data.data[0].id, student_id: this.student.info.id} })
        }
        if (type === 'addfee') {
          this.$router.push({ name: this.source === 'student' ? 'studentAddFee' : 'addFee', query: {detail: true, class_id: data.class_id, student_id: this.student.info.id} })
        }
        if (type === 'refund') {
          this.$router.push({ name: this.source === 'student' ? 'studentQuitClass' : 'quitClass', query: {detail: true, class_id: data.class_id, enroll_id: data.data[0].id, student_id: this.student.info.id} })
        }
          ...
          if (res.ok) {
            ...
            res.data.map((v) => {
              let obj = {
                pay_time: v.pay_time,
                amount: v.amount,
                pay_option_price: v.pay_option_price,
      ...
      goDetailPage (val) {
        this.detailPageConfig.page = val
        this.getEntryInfo()
      }
    }
  }
