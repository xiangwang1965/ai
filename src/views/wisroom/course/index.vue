<template>
  <div class="courseView">
    <div class="cac-bread-bar courseHeader" style="height:auto;">
      ...
      <el-button type="primary" class="clearfix cac-button-one create-button create-course" size="small" round icon="el-icon-plus" @click="viewChooseTypeDialog">创建课程</el-button>
    ...
    <div class="tableWrapper" id="tableWrapper">
      <el-table
        ...
        <el-table-column :resizable="false" align="center" label="班型" prop="" show-overflow-tooltip>
        ...
        <el-table-column :resizable="false" align="center" label="操作" width="180">
          <template slot-scope="scope">
            <el-button type="primary" class="cac-button-two" size="mini" round @click="goDetail(scope.$index, scope.row)">详情</el-button>
            <el-button type="primary" class="cac-button-two admin-able" size="mini" round @click="handleDisable(scope.$index, scope.row)">{{scope.row.valid==0?'启用':'禁用'}}</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    goDetail (index, item) {
      this.$router.push({ name: 'wisroomCourseDetail', query: { base_class_id: item.id } })
    },
    ...
    handleDetail (index, row) {
      this.$router.push({ name: 'wisroomCourseDetail', query: { class_id: row.class_id, class_type: row.class_type } })
    },
