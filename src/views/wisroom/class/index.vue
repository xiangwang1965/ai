<template>
  <div class="wisroomClass">
    <div class="cac-bread-bar">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item class="cac-bread">班级管理</el-breadcrumb-item>
      </el-breadcrumb>
      <router-link :to="{name: 'createWisroomClass', query: {create: 'create', is_teach: is_teach,is_live: is_live}}">
    ...
    <div class="selectRow">
      ...
      <el-button style="vertical-align:top;" type="primary" class="cac-button-one spc_filter" size="small" round @click="resetPage">筛选</el-button>
      <el-input
        ...
        size="small"
        round>
        <i slot="suffix" class="el-icon-search el-input__icon" @click="resetPage"></i>
      </el-input>
    </div>
      ...
      <el-table
        ...
        <el-table-column align="center" label="操作" width="350" show-overflow-tooltip>
          <template slot-scope="scope">
            <router-link style="color:blue;margin-right:10px;" :to="{name: 'wisroomClassDetail', query: { class_id: scope.row.class_id || scope.row.id, class_type: scope.row.class_type }}">
              <el-button type="primary" class="clearfix cac-button-two manage-button" size="mini" round>详情</el-button>
            </router-link>
            <el-button type="primary" class="clearfix cac-button-two manage-button" size="mini" :disabled="scope.row.scene_class_status===3 || scope.row.scene_class_status===4" round @click="handleAddStudent(scope.$index, scope.row)">添加学生</el-button>
            <el-button type="primary" class="clearfix cac-button-two manage-button" size="mini" :disabled="scope.row.scene_class_status===3 || scope.row.scene_class_status===4" round @click="handleEdit(scope.$index, scope.row)">编辑</el-button>
            <el-button type="primary" class="clearfix cac-button-two manage-button" size="mini" :disabled="scope.row.scene_class_status===3 || scope.row.scene_class_status===2 || scope.row.scene_class_status===4" round @click="handleDelete(scope.$index, scope.row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog class="nonDivisionDialog" title="未分班学生" width="1050px" style="height:100%;" :visible.sync="nonDivisionConfig.visible">
      ...
      <el-table :data="nonDivisionList" row-class-name="listTr" v-loading="dialogLoading">
        <el-table-column align="left" prop="name" label="姓名" width="250" show-overflow-tooltip></el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleEdit (index, row) {
      this.$router.push({ name: 'createWisroomClass', query: { create: 'edit', id: row.class_id || row.id, type: row.class_type } })
    },
    // 详情
    ...
    addSingleStudentDone (val) {
      this.$router.push({name: 'wisroomAddStudent', query: {id: val.id, vd_id: val.vd_id, classId: this.class_id, from: 'class'}})
    },
    // 关闭添加学生
    doClose (data) {
